/****************************************************************************** 
  Impl\E9mentation du module image_pbm
******************************************************************************/
...
/* macro donnant l'indice d'un pixel de coordonn\E9es (_x,_y) de l'image _I */
...
/* teste si le fichier d'identificateur f debute par un en-tete
   ...
   - ligne 1 : P1
   - suivie de zero, une ou plusieurs lignes commen\E7ant toutes par #
   La fonction se termine correctement si le fichier est correct, 
   et le pointeur de fichier se trouve \E0 la suite l'entete.
   Sinon, l'ex\E9cution du programme est arretee avec l'affichage d'un message
   d'erreur (appel \E0 ERREUR_FATALE)
    */ 
void entete_fichier_pbm(FILE *f)
{
	...
	/* lecture des \E9ventuelles lignes commen\E7ant par # */
	...
	do
	{
		...
		/* lire un caractere et tester par rapport \E0 '#' */
		char c;
		fscanf(f, "%c", &c);
		if (c=='#')
		{
		...
		{
			/* reculer d'un caract\E8re dans f */
			fseek(f, -1, SEEK_CUR);
...
Image lire_fichier_image(char *nom_f)
  ...
  case '0': I.tab[INDICE_PIXEL(I,j,i)] = BLANC;j++;break;
  case '1': I.tab[INDICE_PIXEL(I,j,i)] = NOIR;j++;break;
  case ' ':case'\n':case'\t' : break;
  default:ERREUR_FATALE("Caractere incorrect");break;
  }
...
void ecrire_image(Image I) {    
	...
	for (int i=1;i<=I.H;i++){
	  for (int j=1;j<=I.L;j++){
		...
		switch(p){
			case NOIR:printf("1");break;

			case BLANC:printf("0");break;
		}
