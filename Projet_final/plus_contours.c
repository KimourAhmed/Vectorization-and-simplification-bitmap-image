...
Liste_tout_les_segments calcul_plusieurs_contours(Image I, Image P, Robot r, double d){
  UINT x;
  UINT y;  
  int nombre_de_segments_apres_simplification=0;
  int nombre_de_segments=0;
  Liste_tout_les_segments segments_simpl= creer_liste_tout_les_segments_vide();
  Plus_contours Liste_des_points_de_contour=creer_liste_Contours_vide();
  
  Contour contour_seq;
  
  Liste_segments liste_segments_un_seul_contour=creer_liste_segments_vide();
  
  Tableau_Point contour_tab; 
  Tableau_Segments contour_simpl_tab;
  ...
  while(tout_blanc==0){
     ...
     liste_segments_un_seul_contour=simplification_douglas_peucker(contour_tab,0 , contour_tab.taille-1 , d);
     
     contour_simpl_tab=sequence_segments_liste_vers_tableau(liste_segments_un_seul_contour);
    
     nombre_de_segments_apres_simplification = nombre_de_segments_apres_simplification+contour_simpl_tab.taille;
     
     segments_simpl=ajouter_element_liste_tout_les_segmments (segments_simpl, contour_simpl_tab);
     
     tout_blanc= trouver_pixel_noir(P,&x,&y); 
     }
  
  printf("nombre de contours:%d\n",Liste_des_points_de_contour.taille);
  printf("nombre_de_segments=%d\n",nombre_de_segments);
  printf("nombre_de_segments_apres_simplification=%d\n",nombre_de_segments_apres_simplification);

  return segments_simpl;
}
